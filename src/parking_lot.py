...
class ParkingLot:
    ...
    def __init__(self, number_of_slots: int) -> None:
        ...
        self.parking_slots = dict((ele, None) for ele in range(1, number_of_slots+1))
    ...
    def park(self, vehicle: Vehicle) -> bool:
        ...
        if self.__is_parking_lot_full():
            print(f"All slots are filled. Please try other Parking lot."
    ...
    def leave_from_slot(self, slot: int) -> bool:
        ...
        if slot > self.number_of_slots or slot <= 0:
            print(f"The {slot} does not exist.")
            return False
    ...
    def __leave(self, slot: int) -> bool:
        vehicle = self.parking_slots.get(slot)
        print(f"Slot Number {slot} vacated, the car with registration number {vehicle.registration_number} left the space, the driver of car was of age {vehicle.driver_age}")
